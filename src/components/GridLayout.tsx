...
export function GridLayout({
  ...
  return <div className="h-full w-full space-y-6">
      ...
      <div className="flex-1 min-h-[500px]">
        ...
        gridTemplateRows: `repeat(${rows}, 1fr)`
          ...
          {Array.from({
          length: totalCells
        }, (_, index) => <Card key={index} className={`
                border-2 border-dashed border-border/50 
                relative group
                transition-all duration-200
                ${editMode ? 'hover:border-primary/50 hover:bg-accent/20 cursor-pointer' : 'opacity-60'}
              ...
              <CardContent className="p-0 flex items-center justify-center h-full relative overflow-hidden">
                ...
                {editMode && <Button size="sm" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 h-8 w-8 p-0 bg-primary hover:bg-primary/90" onClick={e => {
              e.stopPropagation();
      ...
      <div className="text-center space-y-2 mt-6">
        ...
        <p className="text-sm text-muted-foreground/70 max-w-md mx-auto">
          {editMode ? 'Click on any grid slot or the + button to browse components and add them to your dashboard layout.' : 'Enable edit mode to start building your dashboard with the grid layout system.'}
        </p>
      </div>
    </div>;
}